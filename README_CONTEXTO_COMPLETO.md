...
9. **Validación Híbrida Pasiva (JwtAuthGuard)**: El `JwtAuthGuard` está modificado a nivel profundo para permitir "autenticación silenciosa" en endpoints con decorador `@Public()`. Si viene un Bearer Token, lo decodifica y puebla `req.user` sin arrojar Error 401 si falla, posibilitando el uso de endpoints híbridos (como crear órdenes tanto públicas para comensales como privadas para meseros en una misma ruta).
10. **Whitelist ValidationPipe + Snake_case (DTOs)**: El `ValidationPipe` bloquea propiedades no declaradas. Para convivir con apps antiguas o clientes de Android que envían llaves en `snake_case` (ej. `table_id`, `dish_id`), los DTOs alojan intencionalmente como `@IsOptional()` las variables paralelas en `snake_case` y luego los Servicios (como `OrdersService`) gestionan la normalización (`dto.tableId = dto.tableId || dto.table_id`).
11. **Sintaxis TypeORM v0.3+ (Relaciones)**: A causa de configuraciones estrictas, no busques Foreign Keys por nombre en bruto en métodos de búsquedas profundas o relations si tienen un decorador `@ManyToOne()`. En vez de declarar `where: { restaurantId }`, la notación mandatoria de Nest es anidada: `where: { restaurant: { id: restaurantId } }`. Error de incumplir esto: `EntityPropertyNotFoundError`.
